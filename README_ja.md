...
特定のMODプロジェクトに依存せず、すべてのHOI4 MOD開発で汎用的に利用できるスキルがまとめられています。

[English Documentation is here (英語版ドキュメントはこちら)](README.md)
...
本パッケージには、スキルの他に、AIエージェントに読み込ませて開発精度を向上させるための汎用的なHOI4 Mod開発リファレンス資料が `docs/` フォルダに収録されています。
- **`00_character/`**: 指導者・将軍・顧問といったキャラクターシステムの詳細ガイド。
- **`00_coding_contexts/`**: 最適化ガイド、変数操作ガイド、コンソールデバッグコマンドなどの仕様ドキュメント。
- **`01_effects/`**, **`02_scopes/`**, **`04_triggers/`**: HOI4のスコープ、トリガー、効果を構造化した `.json` データベース群。
- **`gitflow.md`**: GitFlowを用いたブランチ運用のガイド。
